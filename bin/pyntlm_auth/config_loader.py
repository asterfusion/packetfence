...
def cleanup_machine_account_binding(force=False):
    ...
    for m in machine_accounts:
        ...
        log.debug(f"cleaning up machine account bind: key = '{key}'")
        try:
            res = redis_client.r.get(key)
        except UnicodeDecodeError:
            log.warning(f"can not decode retrieved value of key: {key}. Check the value and remove the key manually.")
            continue
        except redis.ConnectionError:
            log.warning("redis connection error when trying to bind machine account.")
            continue
        except Exception as e:
            log.warning(f"unexpected error when trying to bind machine account: {type(e)}: {str(e)}")
            continue
        ...
        if not (isinstance(res, str) and res.isdigit()):
            log.warning(f"value of key {key} is not a valid PID. Check the value and remove the key manually.")
            continue

        bind_pid = int(res)

        if not psutil.pid_exists(bind_pid):
            log.debug(f"PID {bind_pid} is already died. Cleaning up.")
            try:
                redis_client.r.delete(key)
            except Exception as e:
                log.warning(f"error occurred when cleaning up machine account binding: key: {key}, err: {str(e)}")
        ...
        try:
            ...
            if process_name != "gunicorn":
                cleanup_flag = True
                log.info(f"process {bind_pid} is not a gunicorn managed process. A clean up will be performed.")
        except psutil.NoSuchProcess:
            log.info(f"no such process with PID: {bind_pid}, probable died right before we check it. Binding removed.")
            cleanup_flag = True
        except psutil.AccessDenied:
            log.info(f"unable to access process with PID: {bind_pid}, this shouldn't happen. Cleaning up anyway.")
            cleanup_flag = True
        except Exception as e:
            log.info(f"error occurred when trying to read process info: pid: {bind_pid}, {str(e)}")
            continue
...
def bind_machine_account(worker_pid):
    ...
    for m in machine_accounts:
        try:
            ...
            log.warning(f"unexpected error when trying to bind machine account: {str(e)}")
...
def config_load():
    ...
    if _LISTEN is None or _LISTEN == "":
        log.critical("parameter LISTEN not found in system environment. unable to start ntlm-auth-api.")
        sys.exit(1)
    ...
    log.debug(f"loading domain config from {conf_dm}")
    try:
    ...
    try:
        ...
        log.warning(f"failed loading additional_machine_accounts: {str(e)}. using 0 as default.")

    if additional_machine_accounts < 0 or additional_machine_accounts > 10:
        additional_machine_accounts = 0
        log.warning(f"invalid additional machine account range (0-10), using 0 as default.")
    ...
    log.info(f"Load database config from {conf_db}")
    try:
        with open(conf_db, 'r') as file:
            cp_db.read_file(file)
    except FileNotFoundError:
        log.critical(f"{conf_db} not found, ntlm-auth-api@{identifier} terminated.")
        sys.exit(1)
    except configparser.Error as e:
        log.critical(f"Error loading {conf_db}: {e}, ntlm-auth-api@{identifier} terminated.")
        sys.exit(1)

    if 'CACHE' not in cp_db:
        log.critical(f"section [CACHE] not found, ntlm-auth-api@{identifier} terminated.")
        sys.exit(1)
    ...
    if c_cache_host is None or c_cache_port is None:
        log.critical(f"unable to load 'CACHE_HOST', 'CACHE_PORT' from config, ntlm-auth-api@{identifier} terminated.")
        sys.exit(1)

    if c_cache_port.isdigit() and 0 < int(c_cache_port) < 65536:
        c_cache_port = int(c_cache_port)
    else:
        log.critical(f"unable to parse CACHE_PORT, value must be a valid port within 1..65535.")
        sys.exit(1)

    log.debug(f"using cache: redis://{c_cache_host}:{c_cache_port}")
    ...
    log.info(f"database config: {c_db_user}:{utils.mask_password(c_db_pass)}@{c_db_host}:{c_db_port}/{c_db}")

    # validate domain.conf
    log.info(f"starting ntlm-auth-api@{identifier}")
    if ad_fqdn == "":
    ...
    if dns_servers != "":
        ...
        if utils.is_ipv4(ad_server):
            config_generator.generate_hosts_entry(ad_server, ad_fqdn)
            log.debug(f"DNS servers not available, Starting using static hosts entry: {ad_server} {ad_fqdn}.")
        else:
            log.critical("Unable to start NTLM Auth API. 'ad_server' must be a valid IPv4 if DNS servers empty.")
            sys.exit(1)
    ...
    log.debug(f"NT Key cache enabled: {nt_key_cache_enabled}")
    ...
    if nt_key_cache_enabled:
        for i in range(1):
            ...
            if nt_key_cache_expire < 60:
                log.warning(f"  NT key cache: expire = '{nt_key_cache_expire}' too small, set to minimum value: 60")
                nt_key_cache_expire = 60
            if nt_key_cache_expire > 864000:
                log.warning(f"NT key cache: expire = '{nt_key_cache_expire}' too large, set to maximum value: 864000")
            ...
            if ad_reset_account_lockout_count_after < 1 or ad_reset_account_lockout_count_after > 99999:
                log.warning(f"NT Key cache: 'ad_reset_account_lockout_counter_after' ranges 1..99999, cache disabled.")
                nt_key_cache_enabled = False
                break
            ...
            if max_allowed_attempts_per_device < 0 or max_allowed_attempts_per_device > 999:
                log.warning(f"NT Key cache: '{s_device}' ranges 0..999, set to '{s_threshold}' by default.")
                break
            if max_allowed_attempts_per_device > ad_account_lockout_threshold:
                log.warning(f"NT Key cache: '{s_device}' > '{s_threshold}', set to '{s_threshold}' by default.")
